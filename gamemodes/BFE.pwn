...
include < a_samp >
    ...
    #include < sscanf2 > #include < zcmd > #include < mselection > #include < GetVehicleColor > #include < gettime > #include < foreach > #include < Samp_Fixer / fix_attachments >

    main() {}

#
define MAX_OSLOTS 11# undef MAX_PLAYERS# define MAX_PLAYERS 50
...
#
define COLOR_PURPLE 0xD526D9FF# define COLOR_FIREBRICK 0xB22222FF# define COLOR_LIGHTGREEN 0x00FF00FF# define COLOR_PINK 0xFFB6C1FF# define COLOR_LIGHTBLUE 0x33CCFFAA# define COLOR_GREY 0xAFAFAFAA# define COLOR_WHITE 0xFFFFFFFF# define COLOR_ORANGE 0xFF8000FF# define COLOR_YELLOW 0xFFFF00FF# define COLOR_BLUE 0x0080FFC8# define COLOR_RED 0xFF0000FF# define COLOR_LIME 0x00FF00FF# define COLOR_GREEN 0x33AA33AA# define COLOR_VIOLET 0xEE82EEFF
...
native WP_Hash(buffer[], len,
    const str[]);


static
const Float: MBSPAWN[][mbinfo] = {
...
//RANDOM SPAWNS//////////////////////////////////////////////////////////
static
const Float: RandomPlayerSpawns[][7] = {
    {
        1320.7914,
        1262.0884,
        10.8203,
...
public MoneyBag() {
    ...
    if (!MoneyBagFound) {
        format(string, sizeof(string), ""
            RED "The "
            PINK " Money bag "
            RED "has  not  been found, it is still hidden in "
            BLUE " %s", MoneyBagLocation);
...
public OnPlayerPickUpPickup(playerid, pickupid) {
    if (pickupid == MoneyBagPickup) {
        ...
        format(string, sizeof(string), ""
            ...
            CYAN " that had "
            GREEN "$%d "
            CYAN "inside, located in "
            PINK "%s", pname, money, MoneyBagLocation);
        MoneyBagFound = 1;
        SendClientMessageToAll(-1, string);
        DestroyPickup(MoneyBagPickup);
        SendClientMessage(playerid, -1, ""
            RED "You have found the "
            ORANGE " Money Bag!");
        ...
        foreach(new i: Player) {
            if (P_INFO[i][spectatorid] == playerid) {
                format(strg, sizeof(strg), ""
                    BLUE "- AS - {%06x}%s(%d) "
                    BLUE "died while you were spectating, you may resume spectating by using /spec again.", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid);
...
public OnPlayerConnect(playerid) {
    ...
    if (P_INFO[playerid][Banned] == 1) {
        ...
        format(str1, sizeof(str1), ""
            ORANGE "You are banned from the server!\nBan Time: "
            GREY "Permanent\n"
            RED "Admin who banned you: "
            GREY "%s\n"
            ORANGE "Reason: "
            GREY "%s\n\n{F2DD3F}Were you banned unfairly or do you want to get unbanned? Make an unban appeal at "
            GREEN "bfesamp.tk{F2DD3F}!", P_INFO[playerid][BanAdmin], P_INFO[playerid][BanReason]);
        ShowPlayerDialog(playerid, DIALOG_BAN, DIALOG_STYLE_MSGBOX, ""
            RED "You are banned!", str1, "OK", "");
    ...
    if (P_INFO[playerid][BanExpire] != 0) {
        ...
        if (gettime() > P_INFO[playerid][BanExpire]) {
            ...
            SendClientMessage(playerid, -1, ""
                CYAN "SERVER: Your ban has expired. You are now unbanned from the server.");
        }
        format(str2, sizeof(str2), ""
            ORANGE "You are banned from the server!\nBan Expire: "
            GREY "%i/%i/%i [DD/MM/YY] | %i:%i\n"
            ORANGE "Admin who banned you: "
            GREY "%s\n"
            RED "Reason: "
            GREY "%s\n\n{F2DD3F}Were you banned unfairly or do you want to get unbanned? Make an unban appeal at "
            GREEN "www.bfesamp.tk{F2DD3F}!", d, m, y, h, mi, P_INFO[playerid][BanAdmin], P_INFO[playerid][BanReason]);
        ShowPlayerDialog(playerid, DIALOG_TEMPBAN, DIALOG_STYLE_MSGBOX, ""
            RED "You are banned!", str2, "OK", "");
        SetTimerEx("KickPlayer", 100, false, "i", playerid);
        return 1;
    }
    new
...
public OnPlayerText(playerid, text[]) {
    if (P_INFO[playerid][Mute] >= 1) {
        new string[50];
        format(string, sizeof(string), ""
            RED "You're muted for another %d seconds.", P_INFO[playerid][Mute]);
    ...
    if (text[0] == '#' && P_INFO[playerid][Admin] >= 1) {
        ...
        format(str, sizeof(str), ""
            RED "[ADMIN CHAT]"
            ORANGE " %s: "
            WHITE "%s", GetName(playerid), text[1]);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
    ...
    switch (dialogid) {
        ...
        case DIALOG_SETTINGS:
            {
                ...
                if (response) {
                    switch (listitem) {
                        case 0:
                            return ShowPlayerDialog(playerid, DIALOG_ACCOUNT, DIALOG_STYLE_LIST, ""
                                ORANGE "BFE Account Settings", "Change Name \nChange Password", "Select", "Cancel");
                        case 1:
                            return ShowPlayerDialog(playerid, PSETTINGS, DIALOG_STYLE_LIST, ""
                                ORANGE "BFE Player Settings", "Change Skin \nPrivate Messages [on/off] \nGoto [on/off] \nGod Mode[on/off]\nChange Nick Color", "Select", "Cancel");
                        case 2:
                            return ShowPlayerDialog(playerid, VSETTINGS, DIALOG_STYLE_LIST, ""
                                ORANGE "BFE Vehicle Settings", "Nitro [on/off] \nAuto Fix [on/off] \nBounce [on/off] \nAnti Fall [on/off] \nTune Your Car ", "Select", "Cancel");
                    }
                }
            }
        case DIALOG_ACCOUNT:
        ...
        case DIALOG_CHANGENAME:
            {
                if (!response) return ShowPlayerDialog(playerid, DIALOG_ACCOUNT, DIALOG_STYLE_LIST, ""
                    ORANGE "BFE Account Settings", "Change Name \nChange Password", "Select", "Cancel");
                if (response) {
                    if (isnull(inputtext)) return SendClientMessage(playerid, -1, ""
                        RED "ERROR: "
                        GREY "You have not specified any new name.");
                    if (strlen(inputtext) < 3 || strlen(inputtext) > 24) return SendClientMessage(playerid, -1, ""
                        RED "ERROR: "
                        GREY "Your new nick cannot be smaller than 3 or longer than 24 characters!");
                    new DBResult: Result, Query[90];
                    format(Query, sizeof(Query), "SELECT username FROM users WHERE username = '%q' LIMIT 0, 1", inputtext);
                    Result = db_query(Database, Query);
                    if (db_num_rows(Result)) return SendClientMessage(playerid, -1, ""
                        RED "ERROR: "
                        GREY "The name you have entered is already registered. Please enter a different name to proceed.");
                    new query[150];
        ...
        case DIALOG_PASS:
            {
                ...
                if (response) {
                    ...
                    db_query(Database, Query);
                    SendClientMessage(playerid, -1, ""
                        ORANGE "You Have successfully changed your password "
                        RED "DONT FORGET IT!");
                }
            }
        case PSETTINGS:
            {
                ...
                if (response) {
                    ...
                    switch (listitem) {
                        ...
                        case 0:
                            return cmd_skin(playerid, "");
                        case 1:
                            return cmd_pms(playerid);
                        case 2:
                            return cmd_gos(playerid);
                        case 3:
                            return cmd_god(playerid);
                        case 4:
                            return cmd_color(playerid, "");
                    }
                }
            }
        ...
        case VSETTINGS:
            {
                ...
                if (response) {
                    switch (listitem) {
                        case 0:
                            return cmd_nitro(playerid);
                        case 1:
                            return cmd_autofix(playerid);
                        case 2:
                            return cmd_vjump(playerid);
                        case 3:
        ...
        case WEAPON_DIALOG_MAIN:
            {
                if (response) {
                    switch (listitem) {
                        ...
                        case 0:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_MEELE, DIALOG_STYLE_LIST, "Weapon Menu", "Knuckle Dusters\nGolf Club\nNite Stick\nKnife\nBat\nShovel\nPool Cue\nKatana\nDildo\nSpray Can\n", "Select", "Cancel");

                        case 1:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_HANDGUNS, DIALOG_STYLE_LIST, "Weapon Menu", "9mm\nSilenced 9mm\nDesert Eagle", "Get", "Cancel");

                        case 2:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_MACHINEGUNS, DIALOG_STYLE_LIST, "Weapon Menu", "Uzi\nSMG\nTec9", "Get", "Cancel");

                        case 3:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_RIFLES, DIALOG_STYLE_LIST, "Weapon Menu", "Country Rifle\nSniper Rifle", "Get", "Cancel");

                        case 4:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_ASSAULT, DIALOG_STYLE_LIST, "Weapon Menu", "AK-47\nM4", "Get", "Cancel");

                        case 5:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_SHOTGUNS, DIALOG_STYLE_LIST, "Weapon Menu", "Standard Shotgun\nSawnoff Shotgun\nCombat Shotgun", "Get", "Cancel");

                        case 6:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_THROWN, DIALOG_STYLE_LIST, "Weapon Menu", "Granade\nTear Gas", "Get", "Cancel");

                        case 7:
                            return ShowPlayerDialog(playerid, WEAPON_DIALOG_OTHER, DIALOG_STYLE_LIST, "Weapon Menu", "Fire Extinguisher\nSatchel Charge\nParachute\nNightvision Goggles\nFlowers", "Get", "Cancel");
        ...
        case WEAPON_DIALOG_MEELE:
            {
                if (!response) return ShowPlayerDialog(playerid, WEAPON_DIALOG_MAIN, DIALOG_STYLE_LIST, ""
                    RED "BFEWeapon Lists", "{00FF00}Melees\n{8000FF}Pistols\n{FF8000}Sub-Machine Guns\n{FF00FF}Rifle's\n{FF0080}Assault Rifle\n{DFFF00}Shotgun\n{8080FF}Thrown\n{1340F2}Other", "Select", "Cancel");
                if (response) {
                    switch (listitem) {
                        case 0:
                            {
        ...
        case DIALOG_VEHICLES:
            {
                if (response) {
                    switch (listitem) {
                        ...
                        case 0:
                            return ShowModelSelectionMenu(playerid, bikelist, "Select Vehicle");
                        case 1:
                            return ShowModelSelectionMenu(playerid, Industrial, "Select Vehicle");
                        case 2:
                            return ShowModelSelectionMenu(playerid, Public, "Select Vehicle");
                        case 3:
                            return ShowModelSelectionMenu(playerid, Helicopters, "Select Vehicle");
                        case 4:
                            return ShowModelSelectionMenu(playerid, Airplanes, "Select Vehicle");
                        case 5:
                            return ShowModelSelectionMenu(playerid, RC, "Select Vehicle");
                        case 6:
                            return ShowModelSelectionMenu(playerid, Trailers, "Select Vehicle");
                        case 7:
                            return ShowModelSelectionMenu(playerid, Unique, "Select Vehicle");
                        case 8:
                            return ShowModelSelectionMenu(playerid, Saloons, "Select Vehicle");
                        case 9:
                            return ShowModelSelectionMenu(playerid, Boats, "Select Vehicle");
                        case 10:
                            return ShowModelSelectionMenu(playerid, Lowriders, "Select Vehicle");
                        case 11:
                            return ShowModelSelectionMenu(playerid, Off, "Select Vehicle");
                        case 12:
                            return ShowModelSelectionMenu(playerid, Sports, "Select Vehicle");
        ...
        case DIALOG_DM:
            {
                if (response) {
                    switch (listitem) {
                        case 0:
                            return cmd_de(playerid);
                        case 1:
                            return cmd_rw(playerid);
                        case 2:
                            return cmd_sos(playerid);
                        case 3:
                            return cmd_sniperdm(playerid);
                        case 4:
        ...
        case DIALOG_RADIO:
            {
                if (response) {
                    switch (listitem) {
                        case 0:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=40568");

                        case 1:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=48370");

                        case 2:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=5661");

                        case 3:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=86433");

                        case 4:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=333407");

                        case 5:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=31645");

                        case 6:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=19275");

                        case 7:
                            return PlayAudioStreamForPlayer(playerid, "http://yp.shoutcast.com/sbin/tunein-station.pls?id=105867");

                        case 8:
                            return PlayAudioStreamForPlayer(playerid, "http://radio-pls.rtl.fr/fun_hq.pls");
        ...
        case 1111:
            {
                if (response) {
                    ...
                    switch (listitem) {
                        case 0:
                            AddVehicleComponent(vehid, 1010), CarmodDialog(playerid), PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                        case 1:
                        ...
                        case 3:
                            AddVehicleComponent(vehid, 1086), PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0), CarmodDialog(playerid);
                        case 4:
                            AddVehicleComponent(vehid, 1087), PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0), CarmodDialog(playerid);
                        case 5:
                            {
                                switch (vehmd) {
        ...
        case 1114:
            {
                ...
                if (response) {
                    switch (listitem) {
                        case 0:
                            ShowPlayerDialog(playerid, 1115, DIALOG_STYLE_LIST, "Choose a paintjob:", "Paintjob 1\nPaintjob 2\nPaintjob 3", "Select", "Cancel");
                        case 1:
                            Mod(playerid);
                        case 2:
                            Mod1(playerid);
                ...
                if (response) {
                    switch (listitem) {
                        ...
                        case 2:
                            Mod3(playerid);
                        case 3:
                            AddVehicleComponent(vehid, 1122), AddVehicleComponent(vehid, 1101), ShowPlayerDialog(playerid, 1003, DIALOG_STYLE_LIST, "Choose one", "Paintjob\nChrome\nSlamin\nFlame Sideskirt \nArches Sideskirt Roof\n{FF0000}Back", "Select", "Cancel");
                        case 4:
                            AddVehicleComponent(vehid, 1106), AddVehicleComponent(vehid, 1124), ShowPlayerDialog(playerid, 1003, DIALOG_STYLE_LIST, "Choose one", "Paintjob\nChrome\nSlamin\nFlame Sideskirt \nArches Sideskirt Roof\n{FF0000}Back", "Select", "Cancel");
                        case 5:
                            CarmodDialog(playerid);
                    }
                }
                        ...
                        case 1:
                            {
                                switch (vehmd) {
                                    ...
                                    default:
                                        return ShowPlayerDialog(playerid, 1020, DIALOG_STYLE_LIST, "Hood", "Champ\nFury\nRace\nWorx\n{FF0000}Back", "Select", "Cancel");
                                }
                            }
                        case 2:
                            {
        ...
        case DIALOG_CMDS:
            {
                if (response) {
                    switch (listitem) {
                        ...
                        case 0:
                            return ShowPlayerDialog(playerid, DIALOG_CMDS_ACCOUNT, DIALOG_STYLE_MSGBOX, ""
                                RED "BFE Account Commands", ""
                                ORANGE "/changename "
                                GREEN "- change your name\n"
                                ORANGE "/changepass  "
                                GREEN "- change your account password", "OK", "");
                        case 1:
                            {
                                new bigstring[600];
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /dm              "
                                    PINK "       - "
                                    CYAN "Show lists of DM areas \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /t               "
                                    PINK "       - "
                                    CYAN "Shows Thelist of teleports \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /pm              "
                                    PINK "       - "
                                    CYAN "Send a private message \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /o               "
                                    PINK "       - "
                                    CYAN "To Attach Objects to your skin \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /mymute          "
                                    PINK "       - "
                                    CYAN "View your mute clock \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /mutelist        "
                                    PINK "       - "
                                    CYAN "View list of muted players \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /settings        "
                                    PINK "       - "
                                    CYAN "View and change your account preferences \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /pms             "
                                    PINK "       - "
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /god             "
                                    PINK "       - "
                                    CYAN "Toggle god mode \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /givemoney       "
                                    PINK "       - "
                                    CYAN "Transfer your money to another player \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /goto            "
                                    PINK "       - "
                                    CYAN "Teleport to someother player \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /gos             "
                                    PINK "       - "
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /w               "
                                    PINK "       - "
                                    CYAN "Change your weapon \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /color           "
                                    PINK "       - "
                                    CYAN "Change your nick name colour \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /report          "
                                    PINK "       - "
                                    CYAN "Report an hacker/cheater to administrators \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /admins          "
                                    PINK "       - "
                                    CYAN "View online administrators \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /skin            "
                                    PINK "       - "
                                    CYAN "Change your player skin  \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /kill            "
                                    PINK "       - "
                                    CYAN "Suicide yourself \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /para            "
                                    PINK "       - "
                                    CYAN "Get a parachute \n");

                                ShowPlayerDialog(playerid, DIALOG_CMDS_PLAYER, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE Player Commands", bigstring, "OK", "");
                            }
                        ...
                        case 2:
                            {
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /tune            "
                                    PINK "      - "
                                    CYAN "Tune Your Vehicle\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /autofix         "
                                    PINK "      - "
                                    CYAN "Toggles autofix on/off\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /vjump           "
                                    PINK "      - "
                                    CYAN "Toggles Vehicle Bounce on/off\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /nitro           "
                                    PINK "      - "
                                    CYAN "Toggles Nitro Boost on/off\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /anitfall        "
                                    PINK "      - "
                                    CYAN "Toggles AntiFall on/off\n");
                                ShowPlayerDialog(playerid, DIALOG_CMDS_VEHICLE, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE Vehicle Commands", bigstring, "OK", "");
                            }
                        ...
                        case 3:
                            {
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /sellhouse       "
                                    PINK "      - "
                                    CYAN "To sell your house\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /lock            "
                                    PINK "      - "
                                    CYAN "Lock your house\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /unlock          "
                                    PINK "      - "
                                    CYAN "Unlock your house\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /gohome          "
                                    PINK "      - "
                                    CYAN "Teleport to your house\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /getcar          "
                                    PINK "      - "
                                    CYAN "Get house vehicle\n");
                                ShowPlayerDialog(playerid, DIALOG_CMDS_HOUSE, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE House Commands", bigstring, "OK", "");
                        ...
                        case 4:
                            return SendClientMessage(playerid, -1, ""
                                GREEN "VIP"
                                RED "Features are not available right now !");
                        case 5:
                            return ShowPlayerDialog(playerid, DIALOG_CMDS_ADMIN, DIALOG_STYLE_LIST, ""
                                RED "BFE Admin Commands", "Level 1\nLevel 2\nLevel 3\nLevel 4\nLevel 5\n", "Select", "Cancel");
        ...
        case DIALOG_CMDS_ADMIN:
            {
                if (!response) {
                    ...
                    switch (listitem) {
                        case 0:
                            {
                                if (P_INFO[playerid][Admin] < 1) return SendClientMessage(playerid, -1, ""
                                    RED "ERROR:"
                                    GREY "You are not authorised to access this section!");
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /ban             "
                                    PINK "      - "
                                    CYAN "Ban a Rule breaker\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /spec            "
                                    PINK "      - "
                                    CYAN "Spectate a player\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /specoff         "
                                    PINK "      - "
                                    CYAN "Stop spectating\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /warn            "
                                    PINK "      - "
                                    CYAN "Warn a Rule breaker\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /tempban         "
                                    PINK "      - "
                                    CYAN "Ban a Rule breaker temporary\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /jail            "
                                    PINK "      - "
                                    CYAN "Jail the Rule Breaker\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /unjail          "
                                    PINK "      - "
                                    CYAN "Unjail the jailed one\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /mute            "
                                    PINK "      - "
                                    CYAN "Shut the mouth of Rule breaker \n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /unmute          "
                                    PINK "      - "
                                    CYAN "Un mute the muted one\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /gethere         "
                                    PINK "      - "
                                    CYAN "Get a plaer to Admin's location\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /announce        "
                                    PINK "      - "
                                    CYAN "Make an Announcement\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /kick            "
                                    PINK "      - "
                                    CYAN "Kick a Rule breaker\n");

                                ShowPlayerDialog(playerid, ADMIN1, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE Level 1 Admin Commands", bigstring, "OK", "");
                            }
                        case 1:
                            {
                                new bigstring[600];

                                if (P_INFO[playerid][Admin] < 2) return SendClientMessage(playerid, -1, ""
                                    RED "ERROR:"
                                    GREY "You are not authorised to access this section!");
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /freeze          "
                                    PINK "      - "
                                    CYAN "Freeze a Rule breaker\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /unfreeze        "
                                    PINK "      - "
                                    CYAN "Un Freeze the frozen one\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /disarm          "
                                    PINK "      - "
                                    CYAN "Reset a players weapon\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /worldtime       "
                                    PINK "      - "
                                    CYAN "Change server time\n");
                                strcat(bigstring, ""
                                    RED "+Level 1 commands");
                                ShowPlayerDialog(playerid, ADMIN2, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE Level 2 Admin Commands", bigstring, "OK", "");
                            }
                        case 2:
                            {
                                new bigstring[600];
                                if (P_INFO[playerid][Admin] < 3) return SendClientMessage(playerid, -1, ""
                                    RED "ERROR:"
                                    GREY "You are not authorised to access this section!");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /resetgod          "
                                    PINK "      - "
                                    CYAN "Reset everyone's god mode\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /unmuteall         "
                                    PINK "      - "
                                    CYAN "Unmute all mute players\n");
                                strcat(bigstring, ""
                                    RED "+Level 1 and Level 2 commands");
                                ShowPlayerDialog(playerid, ADMIN3, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE Level 3 Admin Commands", bigstring, "OK", "");
                            }
                        case 3:
                            {
                                new bigstring[600];
                                if (P_INFO[playerid][Admin] < 4) return SendClientMessage(playerid, -1, ""
                                    RED "ERROR:"
                                    GREY "You are not authorised to access this section!");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /gotomb            "
                                    PINK "      - "
                                    CYAN "Goto moneybag location\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /togglemb          "
                                    PINK "      - "
                                    CYAN "Turn Money Bag on/off\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /startmb           "
                                    PINK "      - "
                                    CYAN "Start Moneybag\n");
                                strcat(bigstring, ""
                                    RED "+Level 1  Level 2 and Level 3 commands");
                                ShowPlayerDialog(playerid, ADMIN4, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE Level 4 Admin Commands", bigstring, "OK", "");
                            }
                        case 4:
                            {
                                new bigstring[600];
                                ...
                                strcat(bigstring, ""
                                    ...
                                    YELLOW " /kickall           "
                                    PINK "      - "
                                    CYAN "Kick all player out of server\n");
                                strcat(bigstring, ""
                                    ORANGE " •"
                                    YELLOW " /setlevel          "
                                    PINK "      - "
                                    CYAN "Set Players Admin Level\n");
                                strcat(bigstring, ""
                                    RED "+Level 1  Level 2  Level 3 and Level 4 commands");
                                ShowPlayerDialog(playerid, ADMIN5, DIALOG_STYLE_MSGBOX, ""
                                    RED "BFE Level 4 Admin Commands", bigstring, "OK", "");
                            }
                    }
                }
        ...
        case DIALOG_TELES:
            {
                if (response) {
                    ...
                    switch (listitem) {
                        case 0:
                            return cmd_aa(playerid);
                        case 1:
                            return cmd_lsa(playerid);
                        case 2:
                            return cmd_lva(playerid);
                        case 3:
                            return cmd_sfa(playerid);
                        case 4:
                            return cmd_mc(playerid);
                        case 5:
                            return cmd_ls(playerid);
                        case 6:
                            return cmd_lv(playerid);
                        case 7:
                            return cmd_sf(playerid);
                        case 8:
                            return cmd_trans(playerid);
                        case 9:
                            return cmd_arch(playerid);
                    }
                }
            }
        ...
        case DIALOG_ATTACH_INDEX_SELECTION:
            {
                if (response) {
                    if (IsPlayerAttachedObjectSlotUsed(playerid, listitem)) {
                        ShowPlayerDialog(playerid, DIALOG_ATTACH_EDITREPLACE, DIALOG_STYLE_MSGBOX, "Player Objects/Attachment: (Delete/Edit)", ""
                            WHITE "Do you wish to edit the attachment in that slot, or delete it?", "Edit(!)", "Delete(X)");
                    } else {
                        ShowPlayerDialog(playerid, DIALOG_ATTACH_OBJECT_SELECTION, DIALOG_STYLE_LIST, "Player Objects/Attachment: (Select Object Path)", "Path:1 :: "
                            GREY "Server Objects Menu\n"
                            WHITE "Path:2 :: "
                            GREY "Custom Object", "Next(>>)", "Back(<<)");
                    }
        ...
        case DIALOG_ATTACH_OBJECT_SELECTION:
            {
                ...
                if (response) {
                    ...
                    if (listitem == 1) ShowPlayerDialog(playerid, DIALOG_ATTACH_OBJECT2_SELECTION, DIALOG_STYLE_INPUT, "Player Objects/Attachment: (Insert objectid)", ""
                        WHITE "Put your custom objectid below, You can also take help from ''http://wiki.sa-mp.com''.", "Edit", "Back(<<)");
                }
            }
        case DIALOG_ATTACH_OBJECT2_SELECTION:
            {
                ...
                if (response) {
                    if (!strlen(inputtext)) return SendClientMessage(playerid, -1, "PLAYER: You can't leave the coloumn blank."), ShowPlayerDialog(playerid, DIALOG_ATTACH_OBJECT2_SELECTION, DIALOG_STYLE_INPUT, "Player Objects/Attachment: (Insert objectid)", ""
                        WHITE "Put your custom objectid below, You can also take help from ''http://wiki.sa-mp.com''.", "Edit", "Back(<<)");
                    if (!IsNumeric(inputtext)) return SendClientMessage(playerid, -1, "PLAYER: You can't fill a object name, only object id's allowed."), ShowPlayerDialog(playerid, DIALOG_ATTACH_OBJECT2_SELECTION, DIALOG_STYLE_INPUT, "Player Objects/Attachment: (Insert objectid)", ""
                        WHITE "Put your custom objectid below, You can also take help from ''http://wiki.sa-mp.com''.", "Edit", "Back(<<)");
                    new obj;
...
CMD: changename(playerid) {
    ...
    if (!P_INFO[playerid][Logged]) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You must be logged in!");
    ShowPlayerDialog(playerid, DIALOG_CHANGENAME, DIALOG_STYLE_INPUT, ""
        ORANGE "Change your nick", ""
        GREY "Please enter your new name:\nYour name cannot be smaller than 3 or longer than 24 characters.", "Change", "Cancel");
    return 1;
}
...
CMD: changepass(playerid) {
    ...
    if (!P_INFO[playerid][Logged]) return SendClientMessage(playerid, -1, ""
        RED "You must log on to change password!");
    ShowPlayerDialog(playerid, DIALOG_PASS, DIALOG_STYLE_INPUT, ""
        ORANGE "Change your nick", ""
        GREY "Please enter your new PASSWORD:\nDONT FORGET YOUR PASSWORD AFTER CHANGING IT", "Change", "Cancel");
...
CMD: goto(playerid, params[]) {
    ...
    if (playerid == id) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "You cannot tp to yourself!");
    if (P_INFO[playerid][inDM]) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You cannot use this command here! Please type /leave to exit!");
    if (P_INFO[id][inDM]) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "That player is not in Freeroam");
    if (P_INFO[playerid][Goto] == 0) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "You Turned Off Goto Use "
        RED "/gos");
    if (P_INFO[id][Goto] == 0) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "That player turned off his/her goto");
...
CMD: tune(playerid) {
    ...
    if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER) {
        ...
        RED "ERROR:"
        GREY "You must be in a vehicle to open this dialog!");
    return 1;
...
CMD: top(playerid) {
    ...
    for (new a; a < db_num_rows(result); a++, db_next_row(result)) {
        ...
        format(string, sizeof(string), "%s\n"
            ...
            RED " %s "
            CYAN " scores:"
            ORANGE " %i", string, a + 1, name, scores);

    }
    ShowPlayerDialog(playerid, DIALOG_TOP, DIALOG_STYLE_MSGBOX, ""
        RED "Top Players ", string, "Close", "");
...
CMD: pm(playerid, params[]) {
    ...
    if (sscanf(params, "us", id, str2)) {
        SendClientMessage(playerid, -1, ""
            ORANGE "Usage: /pm <id> <message>");
        return 1;
    }
    if (!IsPlayerConnected(id)) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "Player not connected");
    if (playerid == id) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "You cannot pm yourself!");
    if (P_INFO[playerid][pms] == 0) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "You Turned Off PMS Use "
        RED "/pms");
    if (P_INFO[id][pms] == 0) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "That Player blocked his/her pms");
    else {
...
CMD: para(playerid) {
    GivePlayerWeapon(playerid, 46, 1);
    SendClientMessage(playerid, -1, ""
        CYAN "You Recieved a parachute!");
    return 1;
}
CMD: v(playerid, params[]) {
    if (GetPlayerVirtualWorld(playerid) == 1520 || GetPlayerVirtualWorld(playerid) == 1517) return SendClientMessage(playerid, -1, ""
        GREY "You can't use that cmd in this zone!");
    ...
    if (sscanf(params, "s[32]D(1)D(1)", Vehicle, ColorOne, ColorTwo)) {
        ShowPlayerDialog(playerid, DIALOG_VEHICLES, DIALOG_STYLE_LIST, ""
            ORANGE "BFE Vehicle Spawner", "Bikes\nIndustrial\nPublic Service\nHelicopters\nPlanes\nRC Vehicles\nTrailers\nUnique\nSaloons\nBoats\nLowriders\nOff Road\nSports", "Select", "Cancel");
        return 1;
    ...
    if (GetPlayerVirtualWorld(playerid) != 1518) {
        if (strcmp(Vehicle, "520") == 0 || strcmp(Vehicle, "425") == 0 || strcmp(Vehicle, "432") == 0 || strcmp(Vehicle, "464") == 0 ||
            strcmp(Vehicle, "476") == 0 && GetPlayerVirtualWorld(playerid) == 0) return SendClientMessage(playerid, -1, ""
            RED "ERROR: "
            GREY "The vehicle you have specified is globally disabled.");
        ...
        if (VehicleID != 425 && VehicleID != 432 && VehicleID != 520) {
            if (VehicleID == -1) {
                ...
                if (VehicleID < 400 || VehicleID > 611) {
                    return SendClientMessage(playerid, -1, ""
                        RED "ERROR: "
                        GREY "You have specified an invalid vehicle name!");
                }
            }
        ...
        SendClientMessage(playerid, -1, ""
            RED "ERROR: "
            GREY "You cannot spawn a vehicle in this area.");
    }
    if (GetPlayerVirtualWorld(playerid) == 1518) {
        VehicleID = GetVehicleModelIDFromName(Vehicle);
        if (VehicleID == -1) {
...
CMD: antifall(playerid) {
    if (P_INFO[playerid][Logged] == false) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You must be logged in!");
    if (P_INFO[playerid][AntiFall] == 1) {
...
CMD: autofix(playerid) {
    if (P_INFO[playerid][Logged] == false) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You must be logged in!");
    if (P_INFO[playerid][inDM] == true) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You cannot use this command in DM arena! Type /leave to exit!");
    if (P_INFO[playerid][AutoRepair] == 1) {
...
CMD: nitro(playerid) {
    if (P_INFO[playerid][Spawned] == 0) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You must be logged in!");
...
CMD: settings(playerid) {
    ...
    ShowPlayerDialog(playerid, DIALOG_SETTINGS, DIALOG_STYLE_LIST, ""
        ORANGE "BFE Player Settings", "Account Settings \nPlayer Settings \nVehicle Settings", "Select", "Cancel");
...
CMD: t(playerid) {
    if (P_INFO[playerid][Logged] == false) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "YOU MUST LOGIN !");
    ShowPlayerDialog(playerid, DIALOG_TELES, DIALOG_STYLE_LIST, ""
        RED "BFE Teleports", "Abandoned Airport(/aa)\nLos Santos Airport (/lsa)\nLas Venturas Airport(/lva)\nSanFierro Airport(/sfa)\nMount Chilliad(/mc)\nLos Santos City(/ls)\nLas VenturasCity(/lv)\nSanFierro City(/sf)\nTransfender(/trans)\nWheel Archs(/arch)\n", "Select", "Cancel");
    return 1;
}
...
CMD: w(playerid) {
    if (P_INFO[playerid][Logged] == false) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "YOU MUST LOGIN !");
    if (P_INFO[playerid][God] == 1) return SendClientMessage(playerid, -1, ""
        RED "ERROR:"
        GREY "TURN OFF GOD MODE FIRST !");
    ShowPlayerDialog(playerid, WEAPON_DIALOG_MAIN, DIALOG_STYLE_LIST, ""
        RED "BFEWeapon Lists", "{00FF00}Melees\n{8000FF}Pistols\n{FF8000}Sub-Machine Guns\n{FF00FF}Rifle's\n{FF0080}Assault Rifle\n{DFFF00}Shotgun\n{8080FF}Thrown\n{1340F2}Other", "Select", "Cancel");
...
CMD: dm(playerid) {
    ...
    ShowPlayerDialog(playerid, DIALOG_DM, DIALOG_STYLE_TABLIST_HEADERS, ""
        RED "BFE Deathmatch", string, "Select", "Cancel");
    return 1;
}
CMD: de(playerid) {
...
CMD: leave(playerid) {
    ...
    if (P_INFO[playerid][inDM] == false) {
        SendClientMessage(playerid, -1, ""
            RED "ERROR: "
            GREY "You are not in a deathmatch arena!");
        return 1;
    }
    if (P_INFO[playerid][inDM] == true) {
        ...
        SendClientMessage(playerid, -1, ""
            RED "[DM] "
            GREY "You have left the deathmatch arena!");
...
CMD: color(playerid, params[]) {
    ...
    if (P_INFO[playerid][Logged] == false) return SendClientMessage(playerid, -1, ""
        ...
        GREY "You must be logged in!");
    if (isnull(params)) {
        ShowPlayerDialog(playerid, DIALOG_COLOR, DIALOG_STYLE_LIST, ""
            VIOLET "Change nick color:", "Purple\nPink\nLight Blue\nLight Green\nGrey\nWhite\nOrange\nYellow\nRed\nGreen", "Change", "Cancel");
        SendClientMessage(playerid, -1, ""
            GREEN "TIP: "
            YELLOW "You can use /color <R> <G> <B> [0-255] to set your own nick color!");
    } else {
        if (sscanf(params, "ddd", R, G, B)) return SendClientMessage(playerid, 0x6FFF00FF, "{F07F1D}USAGE: {BBFF00}/color <R> <G> <B> [0-255]");
        if (R < 0 || R > 255 || G < 0 || G > 255 || B < 0 || B > 255) return SendClientMessage(playerid, -1, ""
            RED "ERROR: "
            GREY "Color R-G-Bs cannot be lower than 0 or higher than 255!");
...
CMD: mymute(playerid, params[]) {
    ...
    new string[50];
    SendClientMessage(playerid, -1, ""
        RED "|__________ My Mute __________|");
    format(string, sizeof(string), ""
        ORANGE "%d "
        BLUE "seconds remain from your mute.", P_INFO[playerid][Mute]);
...
CMD: mutelist(playerid, params[]) {
    ...
    foreach(new i: Player) {
        if (P_INFO[i][Mute] >= 1) {
            ...
            format(string, sizeof(string), ""
                RED "%s (%d) - %d seconds remaining", GetName(i), i, P_INFO[i][Mute]);
            SendClientMessage(playerid, -1, string);
            count++;

        }
    }
    if (count == 0) return SendClientMessage(playerid, -1, ""
        RED "There are no muted players online.");
    if (count == 1) return SendClientMessage(playerid, -1, ""
        YELLOW "There is "
        ORANGE "1"
        YELLOW " player who is muted online.");
    if (count >= 2) {
        format(string, sizeof(string), ""
            GREY "There are "
            RED "%d"
            GREY " amount of players who are online and muted.", count);
        SendClientMessage(playerid, -1, string);
    }
    return 1;
}
...
CMD: admins(playerid) {
    ...
    if (c == 0) return SendClientMessage(playerid, -1, ""
        CYAN "SERVER: There are no administrators online.");
    ShowPlayerDialog(playerid, DIALOG_ADMINS, DIALOG_STYLE_MSGBOX, ""
        RED "Admins Online:", str, "OK", "");
    return 1;
}
...
CMD: report(playerid, params[]) {
    ...
    if (giveid == INVALID_PLAYER_ID) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "Invalid player ID!");
    format(str, sizeof(str), ""
        RED " {%06x}%s(%d) "
        RED "has reported {%06x}%s(%d) "
        RED "for: %s", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid, (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid, reason);
    foreach(new i: Player) {
    ...
    format(str2, sizeof(str2), ""
        BLUE "Your report about {%06x}%s(%d) "
        BLUE "has been sent to %d administrator(s) online.", (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid, count);
...
CMD: moneybag(playerid) {
    new string[150];
    if (!MoneyBagFound) format(string, sizeof(string), ""
        RED "The "
        YELLOW "Money Bag "
        RED "is hidden in "
        BLUE "%s!", MoneyBagLocation);
    if (MoneyBagFound) format(string, sizeof(string), ""
        RED "The "
        WHITE "Money Bag is  not running!");
...
CMD: headshot(playerid) {
    if (P_INFO[playerid][Admin] < 2) return SendClientMessage(playerid, -1, ""
        RED "You are not Authorised to use this Command! ");
    if (hshot) {
        hshot = false;
        SendClientMessageToAll(-1, ""
            RED "Headshot Turned OFF By Admin ");
    } else
    if (!hshot) {
        hshot = true;
        SendClientMessageToAll(-1, ""
            RED "Headshot Turned ON By Admin ");
    }
    return 1;
}
...
CMD: radio(playerid) {
    ...
    ShowPlayerDialog(playerid, DIALOG_RADIO, DIALOG_STYLE_LIST, ""
        YELLOW "Choose any type of music!", ""
        GREY "> Dubstep\n"
        GREY "> HipHop\n"
        GREY "> Reggae\n"
        GREY "> Jazz\n"
...
CMD: kill(playerid) {
    ...
    format(str, sizeof(str), ""
        RED "%s(%d) "
        YELLOW "has commited sucide", GetName(playerid), playerid);
...
CMD: setowner(playerid, params[]) {
    ...
    if (IsPlayerAdmin(playerid)) {
        ...
        format(str, sizeof(str), "{FFE83B}- AS - %s(%d) "
            RED "has been set to Server Owner via RCON.", GetName(playerid), playerid);
        SendClientMessageToAll(-1, str);
        return 1;
    } else return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You are not authorized to use this command!");
...
CMD: ban(playerid, params[]) {
    ...
    db_query(Database, Query);
    format(str, sizeof(str), ""
        GREEN "Administrator {%06x}%s(%d) "
        ORANGE "has permanently banned {%06x}%s(%d)"
        RED "for: %s ", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid, (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid, reason);
    SendClientMessageToAll(-1, str);
    format(banmsg, sizeof(banmsg), ""
        RED "You are banned from the server!\nBan Time: "
        GREY "Permanent\n"
        ORANGE "Admin who banned you: "
        GREY "%s\n"
        RED "Reason: "
        GREY "%s\n{F2DD3F}Were you banned unfairly or do you want to get unbanned? Make an unban appeal at "
        GREEN "bfesamp.tk{F2DD3F}!", GetName(playerid), reason);
    ShowPlayerDialog(giveid, DIALOG_BAN, DIALOG_STYLE_MSGBOX, ""
        RED "You are banned!", banmsg, "OK", "");
...
CMD: spec(playerid, params[]) {
    ...
    if (giveid == INVALID_PLAYER_ID) return GameTextForPlayer(playerid, "~r~Invalid Player ID", 5000, 5);
    if (P_INFO[playerid][IsSpecing] == 1) return SendClientMessage(playerid, -1, ""
        RED "ERROR: "
        GREY "You are alrady spectating a player! Type /specoff to stop.");
    ...
    format(str, sizeof(str), ""
        BLUE "You are spectating {%06x}%s(%d)"
        BLUE ".", (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid);
    SendClientMessage(playerid, -1, str);
...
CMD: warn(playerid, params[]) {
    ...
    format(str, sizeof(str), ""
        BLUE "Administrator {%06x}%s(%d) "
        BLUE "has warned {%06x}%s(%d) "
        BLUE "for: %s", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid, (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid, reason);
...
CMD: tempban(playerid, params[]) {
    ...
    format(str, sizeof(str), ""
        CYAN "Administrator {%06x}%s(%d) "
        CYAN "has temporarily banned {%06x}%s(%d) "
        CYAN "for: %s (%i days)", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid, (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid, reason, days);
    SendClientMessageToAll(-1, str);
    new d, m, y, h, mi, s;
    TimestampToDate(P_INFO[playerid][BanExpire], y, m, d, h, mi, s, 0, 0);
    ...
    format(banmsg, sizeof(banmsg), ""
        ...
        ORANGE "Reason: "
        GREY "%s\n{F2DD3F}Were you banned unfairly or do you want to get unbanned? Make an unban appeal at "
        GREEN "bfesamp.tk{F2DD3F}!", d, m, y, h, mi, GetName(playerid), reason);
    ShowPlayerDialog(giveid, DIALOG_TEMPBAN, DIALOG_STYLE_MSGBOX, ""
        RED "You are banned!", banmsg, "OK", "");
    SetTimerEx("KickPlayer", 500, false, "i", giveid);
    return 1;
}
...
CMD: jail(playerid, params[]) {
    ...
    if (time == 0) return GameTextForPlayer(playerid, "~r~Prison Player for atleast ~n~ 1 Minute", 5000, 5); {
        ...
        format(string, sizeof(string), ""
            ...
            CYAN "for "
            YELLOW "%i"
            CYAN " minutes.", GetName(playerid), GetName(targetid), time);
...
CMD: unjail(playerid, params[]) {
    ...
    format(string, sizeof(string), ""
        ...
        RED "%s "
        CYAN "has unjailed "
        ORANGE "%s.", GetName(playerid), GetName(targetid));
    SendClientMessageToAll(-1, string);
...
CMD: mute(playerid, params[]) {
    ...
    format(string, sizeof(string), ""
        CYAN "%s (%d)"
        YELLOW " was muted for %d minutes by admin "
        RED "%s (%d), "
        ORANGE "Reason:"
        RED " %s", GetName(giveplayerid), giveplayerid, minutes, GetName(playerid), playerid, reason);
    SendClientMessageToAll(-1, string);
...
CMD: unmute(playerid, params[]) {
    ...
    format(string, sizeof(string), ""
        CYAN "%s (%d)"
        YELLOW " was unmuted by "
        RED "%s (%d), "
        ORANGE "Reason: "
        RED "%s", GetName(giveplayerid), giveplayerid, GetName(playerid), playerid, reason);
...
CMD: kick(playerid, params[]) {
        ...
        new str[200];
        format(str, sizeof(str), ""
            BLUE "- AS - Administrator {%06x}%s(%d) "
            BLUE "has kicked {%06x}%s(%d) "
            BLUE "for: %s", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid, (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid, reason);
        SendClientMessageToAll(0xB0C4DEFF, str);
        SetTimerEx("KickPlayer", 500, false, "i", giveid);
        return 1;
...
CMD: unban(playerid, params[]) {
    ...
    fremove(path);
    format(str, sizeof(str), ""
        CYAN "- AS - Administrator {%06x}%s(%d) "
        CYAN "has unbanned %s from the server.", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid, PlayerName);
    SendClientMessageToAll(-1, str);
    return 1;
}
...
CMD: akill(playerid, params[]) {
    ...
    format(str, sizeof(str), ""
        CYAN " Administrator {%06x}%s(%d) "
        CYAN "killed "
        ORANGE "%s(%d)", (GetPlayerColor(playerid) >>> 8), GetName(playerid), playerid, GetName(giveid), giveid);
...
CMD: freeze(playerid, params[]) {
    ...
    SendClientMessage(giveid, -1, ""
        RED "**YOU ARE FREEZED BY ADMIN! ");
...
CMD: unfreeze(playerid, params[]) {
    ...
    SendClientMessage(giveid, -1, ""
        GREEN "**YOU ARE UN FREEZED BY ADMIN! ");
...
CMD: disarm(playerid, params[]) {
    ...
    format(string, sizeof(string), ""
        ...
        RED "%s"
        CYAN " has disarmed "
        YELLOW "%s", GetName(playerid), GetName(targetid));
    SendClientMessageToAll(-1, string);
    ResetPlayerWeapons(targetid);
    return 1;
}
...
CMD: resetgod(playerid) {
    ...
    SendClientMessageToAll(-1, ""
        RED "ADMIN Reseted God Mode of all Players !");
    return 1;
}
...
CMD: kickall(playerid) {
    ...
    SendClientMessageToAll(-1, ""
        RED "ADMIN KICKING EVERYONE FROM SERVER!");
    for (new i = 0; i < MAX_PLAYERS; i++) {
        Kick(i);
    }
    return 1;
...
CMD: gotomb(playerid) {
    ...
    SendClientMessage(playerid, -1, ""
        GREY "You have been teleported to the "
        ORANGE "Money Bag");
    return 1;
        ...
        case 0:
            {
                ...
                format(str, sizeof(str), ""
                    CYAN "- AS - {%06x}%s(%d) "
                    CYAN "has been set to: Regular Player", (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid);
                SendClientMessageToAll(0xB0C4DEFF, str);
                return 1;
            }
        case 1:
            {
                ...
                format(str, sizeof(str), ""
                    CYAN "- AS - {%06x}%s(%d) "
                    CYAN "has been set to: Admin Level 1", (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid);
                SendClientMessageToAll(0xB0C4DEFF, str);
                return 1;
            }
        case 2:
            {
                ...
                format(str, sizeof(str), ""
                    CYAN "- AS - {%06x}%s(%d) "
                    CYAN "has been set to: Admin Level 2", (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid);
                SendClientMessageToAll(0xB0C4DEFF, str);
                return 1;
            }
        case 3:
            {
                ...
                format(str, sizeof(str), ""
                    CYAN "- AS - {%06x}%s(%d) "
                    CYAN "has been set to: Admin Level 3", (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid);
                SendClientMessageToAll(0xB0C4DEFF, str);
                return 1;
            }
        case 4:
            {
                ...
                format(str, sizeof(str), ""
                    CYAN "- Admin - {%06x}%s(%d) "
                    CYAN "has been set to: Server Owner", (GetPlayerColor(giveid) >>> 8), GetName(giveid), giveid);
                SendClientMessageToAll(0xB0C4DEFF, str);
                return 1;
            }
    }
